...
const SectionPage = () => {
  ...
  const handleSaveSection = async () => {
    if (formData.name) {
      try {
        ...
        setFormData({ id: null, name: '' }); // Clear form after saving
      } catch (error) {
  ...
  const handleRemoveSection = async (id) => {
    try {
      await axios.delete(`https://school-backend-1-2xki.onrender.com/api/admin/delete-section/${id}`);
          ...
          <form className="space-y-4">
            <div>
              ...
              <input
                ...
                className="border border-gray-300 rounded-md p-2 w-64 focus:outline-none focus:ring-2 focus:ring-purple-500"
                required
              />
            </div>
