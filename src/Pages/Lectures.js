...
import VideoPopup from "../Student/VideoPopup";
...
const Lectures = () => {
  ...
  useEffect(() => {
    const fetchLectures = async () => {
      ...
      setError("");
      try {
        const response = await fetch("https://school-backend-1-2xki.onrender.com/api/admin/all-lecture");
    ...
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div className={`flex-1 overflow-y-auto transition-all duration-300 ${isSidebarOpen ? "ml-64" : "ml-0"}`}>
        <div className="flex items-center justify-between bg-blue-700 text-white p-4 shadow-lg lg:hidden">
          <h1 className="text-lg font-bold">Lectures</h1>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full bg-white shadow-md rounded-lg">
                ...
                <tbody>
                  {loading ? (
                    <tr>
                      <td colSpan="9" className="text-center p-4">Loading lectures...</td>
                    </tr>
                  ) : lectures.length === 0 ? (
                    <tr>
                      <td colSpan="9" className="text-center p-4">No lectures available</td>
                    </tr>
                  ) : (
                    lectures.map((lecture, index) => (
                      <tr key={lecture._id} className="border-t">
                        ...
                        <td className="px-6 py-4">{lecture.class}</td>
                        <td className="px-6 py-4">{lecture.section}</td>
